...
def get_all_performance(*, session: Session = Depends(db_helper.get_session)):
    performances = session.query(Performance).all()
    return performances


@router.get('/performances/{id}')
def get_performance(*, session: Session = Depends(db_helper.get_session), id: int):
    performance = session.query(Performance).filter_by(id=id).first()
    return performance


@router.post('/performances/', status_code=201)
def add_performance(*, session: Session = Depends(db_helper.get_session), performance_data: PerformanceSchema):
...
def update_performance(*, session: Session = Depends(db_helper.get_session), id: int, performance_data: PerformanceSchema):
    performance = session.query(Performance).filter_by(id=id).first()
...
def delete_performance(*, session: Session = Depends(db_helper.get_session), id: int):
