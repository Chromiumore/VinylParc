...
def get_all_records(*, session: Session = Depends(db_helper.get_session)):
...
def get_record(*, session: Session = Depends(db_helper.get_session), id: int):
    record = session.query(Record).options(selectinload(Record.performances)).filter_by(id=id).first()
    return record


@router.post('/records/', status_code=201)
def add_record(*, session: Session = Depends(db_helper.get_session), record_data: RecordSchema):
...
def update_record(*, session: Session = Depends(db_helper.get_session), id: int, record_data: RecordSchema):
    performances = session.query(Performance).filter(Performance.id.in_(record_data.performances_id)).all()
    record = session.query(Record).options(selectinload(Record.performances)).filter_by(id=id).first()
...
def delete_record(*, session: Session = Depends(db_helper.get_session), id: int):
