...
def test_get_ensembles(client: TestClient, insert_ensembles_data):
    response = client.get('/ensembles')
    data = response.json()

    assert response.status_code == 200
    assert isinstance(data, list)
    for i in range(len(insert_ensembles_data)):
        assert data[i]['id'] is not None
        assert data[i]['name'] == insert_ensembles_data[i].name
        assert data[i]['about'] == insert_ensembles_data[i].about
        assert data[i]['ensemble_type'] == insert_ensembles_data[i].ensemble_type.value
        assert len(data[i]['musicians']) == len(insert_ensembles_data[i].musicians)
